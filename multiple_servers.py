				...
				if dict['command'] == 'new password':
					...
					for dict in message_list:					
						...
						if sender == dict['sender username']:
							...
							valid = False
							if len(db_data) == 0:
								log(f"Message receiver {user2} not found in the database")
							...
							if valid:
								if receiver_server == number:
									log("Message meant for user connected to the server")
									if user2 in name_socket.keys() and validated[name_socket[user2]] == 3:
									...
									log("Message meant for user connected to other server")
									if receiver_server < number:
									...
									for s in other_servers_sockets:
										...
										if r_port == receiver_port:
											s.send(to_send(dict))
							log("Message sent to the appropriate socket")
